...
- [📖 完整使用指南](users/usage.md) - **如果安装后有困惑，从这里开始！**
- [🧠 核心概念](#核心概念)
- [🔌 兼容性与调用](#兼容性与调用)
- [🛠️ 安装](#安装)
- [🧭 集成指南](#集成指南)
...
安装一次；然后使用 [`users/bundles.md`](users/bundles.md) 中的入门包来专注于你的角色。

1. **安装**：

   ```bash
   # 默认：~/.agents/skills（Antigravity 2.0 全局）。使用 --path 指定其他位置。
...
👉 **新功能：** [**完整使用指南 - 请先阅读！**](users/usage.md)（回答："安装后我该做什么？"、"如何执行技能？"、"提示词应该是什么样的？"）

👉 **[完整入门指南](users/getting-started.md)**
...
- [技能 vs MCP 工具](users/skills-vs-mcp-tools.md)
- [捆绑包](users/bundles.md)
- [工作流](users/workflows.md)

## 集成指南

如果你的真实问题是"如何在我的工具中使用 Agentic Awesome Skills？"，请使用相应的指南：

- **[Claude Code](users/claude-code-skills.md)**：安装路径、入门提示词、插件市场流程和首次使用指南。
- **[Cursor](users/cursor-skills.md)**：聊天优先使用、前端/全栈入门技能和实用提示词。
- **[Codex CLI](users/codex-cli-skills.md)**：本地编码工作的规划、实现、调试、测试和审查循环。
- **[Gemini CLI](users/gemini-cli-skills.md)**：广泛的工程、代理系统、集成和 AI 工作流覆盖。
- **[AI 代理技能指南](users/ai-agent-skills.md)**：如何根据更广泛或更窄的替代方案评估此仓库。

## 快速常见问题

### 什么是 Agentic Awesome Skills？

它是一个可安装的 GitHub 库，包含适用于 Claude Code、Cursor、Codex CLI、Gemini CLI、Antigravity 和相关 AI 编码助手的可复用 `SKILL.md` 剧本。

### 如何安装它？

使用 `npx agentic-awesome-skills`，或工具特定标志，如 `--codex`、`--cursor`、`--gemini`、`--claude`、`--antigravity` 或 `--agy`，当你希望安装器定位到特定技能目录时。对于上下文敏感的宿主，还可以用 `--risk`、`--category` 和 `--tags` 安装较小子集。
...
> [!TIP]
> **默认安装器路径**：`~/.agents/skills`（Antigravity 全局）。使用 `--path ~/.agent/skills` 进行工作区特定安装。对于手动克隆，`.agent/skills/` 可用作 Antigravity 的工作区路径。
...
4. 在 [`users/bundles.md`](users/bundles.md) 中浏览入门集合，在 [`users/workflows.md`](users/workflows.md) 中浏览执行剧本。
...
| 工具           | 安装                                                                  | 首次使用                                              |
| -------------- | ------------------------------------------------------------------------ | ------------------------------------------------------ |
| Claude Code    | `npx agentic-awesome-skills --claude` 或 Claude 插件市场 | `>> /brainstorming help me plan a feature`           |
...
| Antigravity    | `npx agentic-awesome-skills --antigravity`                         | `Use @brainstorming to plan a feature`               |
...
- **[Codex CLI 技能](users/codex-cli-skills.md)**：本地编码循环的规划、实现、调试和审查技能。
- **[Gemini CLI 技能](users/gemini-cli-skills.md)**：研究、代理系统、集成和工程工作流的入门栈。
- **[AI 代理技能指南](users/ai-agent-skills.md)**：如何评估技能库、选择广度 vs 精选以及选择正确的起点。
...
- **捆绑包**：[`users/bundles.md`](users/bundles.md) 按角色分组入门技能。
- **工作流**：[`users/workflows.md`](users/workflows.md) 提供逐步执行剧本。
...
| ---------------------- | --------------------------------------------------------- |
| `skills/`            | 规范技能库                               |
| `users/`        | 入门、使用、捆绑包、工作流、可视化指南 |
| `contributors/` | 模板、解剖、示例、质量标准、社区文档 |
| `maintainers/`  | 发布、审计、CI 偏移、元数据维护文档       |
| `sources/`      | 归属和许可参考                      |
...
**常见困惑**："我可以像 `@web-wizard` 一样调用整个捆绑包吗？"

**答案：不能直接这样调用。** 以下是捆绑包的实际含义：

**捆绑包是什么：**

- ✅ 按角色组织的推荐技能列表
- ✅ 可用于 Claude/Codex 插件子集或 Antigravity 激活预设
- ✅ 精选起点，帮助你决定使用什么
- ✅ 发现相关技能的省时捷径
...
1. **安装一次仓库**（你已经拥有所有技能；也可以用插件或激活脚本选择较小子集）
2. **在 [docs/users/bundles.md](users/bundles.md) 中浏览捆绑包**以找到你的角色
3. **从该捆绑包中选择 3-5 个技能**开始在提示词中使用
4. **在与 AI 的对话中引用它们**（例如，"使用 @brainstorming..."）

如果 Antigravity 因活动技能过多而开始达到上下文限制，[`users/agent-overload-recovery.md`](users/agent-overload-recovery.md) 中的可选激活脚本可以仅将你想要的捆绑包或技能 ID 实例化到活动的 Antigravity 目录中。

有关如何实际使用技能的详细示例，请参阅[**使用指南**](users/usage.md)。
...
- [docs/users/workflows.md](users/workflows.md)：人类可读剧本。
...
- **[Agentic Awesome Skills vs Awesome Claude Skills](users/agentic-awesome-skills-vs-awesome-claude-skills.md)** 了解广度 vs 精选列表的权衡。
- **[GitHub 上最佳 Claude Code 技能](users/best-claude-code-skills-github.md)** 了解高意图短名单。
- **[GitHub 上最佳 Cursor 技能](users/best-cursor-skills-github.md)** 了解兼容 Cursor 的选项和选择标准。
...
- 从 [`users/claude-code-skills.md`](users/claude-code-skills.md)、[`users/cursor-skills.md`](users/cursor-skills.md)、[`users/codex-cli-skills.md`](users/codex-cli-skills.md) 和 [`users/gemini-cli-skills.md`](users/gemini-cli-skills.md) 中的工具特定指南开始。
- 从 [`users/bundles.md`](users/bundles.md) 中基于角色的捆绑包开始。
- 在 [`users/workflows.md`](users/workflows.md) 中遵循结果驱动的工作流。
- 使用 [`users/getting-started.md`](users/getting-started.md) 和 [`users/usage.md`](users/usage.md) 中的入门指南。

## 文档

| 对于用户                                                       | 对于贡献者                                                          | 对于维护者                                                                                                                          |
| --------------------------------------------------------------- | ------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- |
| [`users/getting-started.md`](users/getting-started.md) | [`../CONTRIBUTING.md`](../CONTRIBUTING.md)                                       | [`maintainers/release-process.md`](maintainers/release-process.md)                                                              |
| [`users/usage.md`](users/usage.md)                     | [`contributors/skill-anatomy.md`](contributors/skill-anatomy.md) | [`maintainers/audit.md`](maintainers/audit.md)                                                                                  |
| [`users/faq.md`](users/faq.md)                         | [`contributors/quality-bar.md`](contributors/quality-bar.md)     | [`maintainers/ci-drift-fix.md`](maintainers/ci-drift-fix.md)                                                                    |
| [`users/claude-code-skills.md`](users/claude-code-skills.md) · [`users/cursor-skills.md`](users/cursor-skills.md) · [`users/codex-cli-skills.md`](users/codex-cli-skills.md) · [`users/gemini-cli-skills.md`](users/gemini-cli-skills.md) | [`contributors/examples.md`](contributors/examples.md)           | [`maintainers/repo-growth-seo.md`](maintainers/repo-growth-seo.md) · [`maintainers/skills-update-guide.md`](maintainers/skills-update-guide.md) · [`.github/MAINTENANCE.md`](../.github/MAINTENANCE.md) |
| [`users/visual-guide.md`](users/visual-guide.md) · [`users/ai-agent-skills.md`](users/ai-agent-skills.md) · [`users/best-claude-code-skills-github.md`](users/best-claude-code-skills-github.md) · [`users/best-cursor-skills-github.md`](users/best-cursor-skills-github.md) |  |  |
...
如果你有在移除的符号链接变通方法附近创建的旧克隆，请重新安装到新目录或重新运行 `npx agentic-awesome-skills` 安装器。

### Windows 截断或上下文崩溃循环

如果 Antigravity 或基于 Jetski/Cortex 的主机不断重新打开并出现截断错误，请使用专用恢复指南：

- [`users/windows-truncation-recovery.md`](users/windows-truncation-recovery.md)

该指南包括：

- 清理前的备份路径
- 通常需要清除的存储文件夹
- 根据 [issue #274](https://github.com/sickn33/agentic-awesome-skills/issues/274) 改编的可选批处理助手

### Linux 和 macOS 代理过载

如果 Antigravity 仅在同时激活过多技能时变得不稳定，请使用跨平台过载指南：

- [`users/agent-overload-recovery.md`](users/agent-overload-recovery.md)
...
**在线托管：**相同的应用可在 [https://sickn33.github.io/agentic-awesome-skills/](https://sickn33.github.io/agentic-awesome-skills/) 获得，并在每次推送到 `main` 时自动部署。要启用一次：**设置 → 页面 → 构建和部署 → 源：GitHub Actions**。

## 贡献

- 在 `skills/<skill-name>/SKILL.md` 下添加新技能。
- 遵循 [`../CONTRIBUTING.md`](../CONTRIBUTING.md) 中的贡献者指南。
- 使用 [`contributors/skill-template.md`](contributors/skill-template.md) 中的模板。
...
- [Discussions](https://github.com/sickn33/agentic-awesome-skills/discussions) 用于提问、想法、展示帖子和社区反馈。
- [Issues](https://github.com/sickn33/agentic-awesome-skills/issues) 用于可重现的错误和具体的、可操作的改进请求。
...
Made with [contrib.rocks](https://contrib.rocks)。*（图像可能已缓存；[在 GitHub 上查看实时贡献者](https://github.com/sickn33/agentic-awesome-skills/graphs/contributors)。)*
...
原始文档和其他非代码编写内容根据 [CC BY 4.0](../LICENSE-CONTENT) 授权，除非更具体的上游通知另有说明。有关归属和第三方许可证详细信息，请参阅 [docs/sources/sources.md](sources/sources.md)。

---

## 星标历史

[![sickn33/agentic-awesome-skills - Star History Chart](https://api.star-history.com/image?repos=sickn33/agentic-awesome-skills&style=landscape1)](https://star-history.com/sickn33/agentic-awesome-skills)
...
如果 Agentic Awesome Skills 对你有帮助，请考虑 ⭐ 为仓库加星！

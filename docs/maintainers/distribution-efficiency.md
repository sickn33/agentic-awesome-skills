...
explicitly includes `skills/`. The nested `skills/.gitignore` now excludes
`__pycache__/`, `.pyc` and `.pyo` files while preserving Python sources, references,
...
negative control reproduces the root-ignore failure; copying the repository's nested
policy must remove cache files while keeping the legitimate sample bundle. The full
repository pack list must also contain no generated bytecode. See npm's documented
[files and nested-ignore behavior](https://docs.npmjs.com/cli/v11/configuring-npm/package-json/#files).

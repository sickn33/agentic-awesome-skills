...
- for the release-specific Pages build, dispatch only from the exact immutable `vX.Y.Z` release tag and require the tag, package version, and published GitHub Release to identify the same commit before build work begins; ordinary protected pushes to `main` deploy automatically after full Pages validation;

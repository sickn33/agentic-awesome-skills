...
- Runtime packaging now bundles the declared npm dependency closure and promotes only allowlisted runtime assets. The current real tarball is 42,304,319 bytes with npm integrity `sha512-ooLFyRGITWYG+7CnqAZhpx5x8u8A6iODMxpvd077WSBSjxinceA7Bs5glEzlxgr/EN0jH9sO0BqCcCshoGc+wg==`; its 7,237 selected runtime assets total 100,623,648 bytes and bind closure digest `sha256-16f70117660e9ddb72e7b22f2ddbb69ee9716dbabef282a1455cd2e7dc098659`. The tarball is installed into an isolated content-addressed cache, reverified byte-for-byte, and launches `aas-mcp` successfully with `NODE_PATH` cleared. The published `aas` and `aas-mcp` bins are asserted executable on POSIX and smoke-tested from the installed tarball.

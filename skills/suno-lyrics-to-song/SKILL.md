...
description: "Install and use the official Lyrics to Song package, pinned by digest, for paid hosted work on the Beatra service."
category: media
risk: critical
source: community
source_repo: beatra-ai/lyrics-to-song-skill
source_type: official
date_added: "2026-09-18"
author: beatra-ai
tags: [lyrics-to-song, music-generation, mcp, paid-api, beatra]
tools: [claude, codex, cursor, gemini]
license: "MIT-0"
license_source: "https://github.com/beatra-ai/lyrics-to-song-skill/blob/2facaacd68cee88a73847be8563895535c48cd47/LICENSE"
---

# Lyrics to Song Skill

## Overview

Turn full lyrics, a rough draft, or a hook into a structured song with clear sections, style, and vocal direction, from inside Claude Code, Codex, or OpenClaw. The work is produced on the hosted, paid Beatra service
(`mcp.beatra.ai`).
...
| --- | --- |
| Package | `suno-lyrics-to-song` `0.2.0` |
| Archive | `https://cdn.beatra.ai/agent-packages/suno-lyrics-to-song/v0.2.0/suno-lyrics-to-song-skill-0.2.0.zip` |
| Archive SHA-256 | `f264cbd8c426f2c88a75d80b94766187873965f3a7cea6e90a76e06539069a09` |
| Source tree | [`beatra-ai/lyrics-to-song-skill@2facaac`](https://github.com/beatra-ai/lyrics-to-song-skill/tree/2facaacd68cee88a73847be8563895535c48cd47/skills/suno-lyrics-to-song) (full SHA `2facaacd68cee88a73847be8563895535c48cd47`) |
...
- Use when the user explicitly wants Lyrics to Song audio produced on Beatra and accepts that it is paid.
...
curl -fsSLO "https://cdn.beatra.ai/agent-packages/suno-lyrics-to-song/v0.2.0/suno-lyrics-to-song-skill-0.2.0.zip"
printf '%s  %s\n' \
  'f264cbd8c426f2c88a75d80b94766187873965f3a7cea6e90a76e06539069a09' \
  'suno-lyrics-to-song-skill-0.2.0.zip' | shasum -a 256 -c -
```
...
```bash
unzip -l suno-lyrics-to-song-skill-0.2.0.zip
unzip -q suno-lyrics-to-song-skill-0.2.0.zip
find suno-lyrics-to-song -type l -print            # expect no output
find suno-lyrics-to-song -type f -perm -111 -print # expect no output
grep -n '"auto_update": True' suno-lyrics-to-song/scripts/mcp_client.py
```
...
```bash
git clone --quiet --filter=blob:none --no-checkout https://github.com/beatra-ai/lyrics-to-song-skill.git src
git -C src checkout --quiet 2facaacd68cee88a73847be8563895535c48cd47 -- skills/suno-lyrics-to-song
(cd suno-lyrics-to-song && find . -type f | LC_ALL=C sort | xargs shasum -a 256) > archive.sha
(cd src/skills/suno-lyrics-to-song && find . -type f | LC_ALL=C sort | xargs shasum -a 256) > tree.sha
...
```bash
dest="$HOME/.claude/skills/suno-lyrics-to-song"
test ! -e "$dest" || { echo "destination exists; stop and ask the user"; exit 1; }
cp -R suno-lyrics-to-song "$dest"
...
```bash
python3 "$HOME/.claude/skills/suno-lyrics-to-song/scripts/mcp_client.py" verify
```
...
review directory, verify its digest against the matching tree in `beatra-ai/lyrics-to-song-skill`
...
- **Network.** Authorization uses `api.beatra.ai`; every tool call and upload
  ...
  `https://beatra.ai/skills/suno-lyrics-to-song/install.json` and archives from
  `cdn.beatra.ai`.
...
- [Source tree at the pinned commit](https://github.com/beatra-ai/lyrics-to-song-skill/tree/2facaacd68cee88a73847be8563895535c48cd47/skills/suno-lyrics-to-song)
- [Product page](https://beatra.ai/skills/suno-lyrics-to-song)

...
description: "Install and use the official AI Voice Cloning package, pinned by digest, for paid hosted work on the Beatra service."
category: media
risk: critical
source: community
source_repo: beatra-ai/ai-voice-cloning-skill
source_type: official
date_added: "2026-09-18"
author: beatra-ai
tags: [voice-cloning, custom-voice, mcp, paid-api, beatra]
tools: [claude, codex, cursor, gemini]
license: "MIT-0"
license_source: "https://github.com/beatra-ai/ai-voice-cloning-skill/blob/64923d96ecdd1adce28ad4ad264b3808dda0ce2d/LICENSE"
---
...
Create a reusable personal or brand voice from a clean audio sample, give it a memorable name, and hear it in a short test reading, from inside Claude Code, Codex, or OpenClaw. The work is produced on the hosted, paid Beatra service
(`mcp.beatra.ai`).
...
| --- | --- |
| Package | `voice-cloning-studio` `0.2.1` |
| Archive | `https://cdn.beatra.ai/agent-packages/voice-cloning-studio/v0.2.1/voice-cloning-studio-skill-0.2.1.zip` |
| Archive SHA-256 | `38299679b429aa73ce5d514c3e314e77330d7c6d663e480a9d982dd852a428a2` |
| Source tree | [`beatra-ai/ai-voice-cloning-skill@64923d9`](https://github.com/beatra-ai/ai-voice-cloning-skill/tree/64923d96ecdd1adce28ad4ad264b3808dda0ce2d/skills/voice-cloning-studio) (full SHA `64923d96ecdd1adce28ad4ad264b3808dda0ce2d`) |
...
- Use when the user explicitly wants a reusable custom voice enrolled on Beatra and accepts that it is paid.
- Use only after the speaker (or rights holder) attests they own the sample and approve enrollment.
- Do not use to clone a voice without documented owner consent.
...
curl -fsSLO "https://cdn.beatra.ai/agent-packages/voice-cloning-studio/v0.2.1/voice-cloning-studio-skill-0.2.1.zip"
printf '%s  %s\n' \
  '38299679b429aa73ce5d514c3e314e77330d7c6d663e480a9d982dd852a428a2' \
  'voice-cloning-studio-skill-0.2.1.zip' | shasum -a 256 -c -
```
...
```bash
unzip -l voice-cloning-studio-skill-0.2.1.zip
unzip -q voice-cloning-studio-skill-0.2.1.zip
find voice-cloning-studio -type l -print            # expect no output
find voice-cloning-studio -type f -perm -111 -print # expect no output
grep -n '"auto_update": True' voice-cloning-studio/scripts/mcp_client.py
```
...
```bash
git clone --quiet --filter=blob:none --no-checkout https://github.com/beatra-ai/ai-voice-cloning-skill.git src
git -C src checkout --quiet 64923d96ecdd1adce28ad4ad264b3808dda0ce2d -- skills/voice-cloning-studio
(cd voice-cloning-studio && find . -type f | LC_ALL=C sort | xargs shasum -a 256) > archive.sha
(cd src/skills/voice-cloning-studio && find . -type f | LC_ALL=C sort | xargs shasum -a 256) > tree.sha
...
```bash
dest="$HOME/.claude/skills/voice-cloning-studio"
test ! -e "$dest" || { echo "destination exists; stop and ask the user"; exit 1; }
cp -R voice-cloning-studio "$dest"
...
```bash
python3 "$HOME/.claude/skills/voice-cloning-studio/scripts/mcp_client.py" verify
```
...
review directory, verify its digest against the matching tree in `beatra-ai/ai-voice-cloning-skill`
...
- **Network.** Authorization uses `api.beatra.ai`; every tool call and upload
  ...
  `https://beatra.ai/skills/voice-cloning-studio/install.json` and archives from
  `cdn.beatra.ai`.
...
- **Voice enrollment.** Uploading a voice sample creates a persistent custom voice. Require owner-consent attestation before the sample leaves the device.
...
- [Source tree at the pinned commit](https://github.com/beatra-ai/ai-voice-cloning-skill/tree/64923d96ecdd1adce28ad4ad264b3808dda0ce2d/skills/voice-cloning-studio)
- [Product page](https://beatra.ai/skills/voice-cloning-studio)

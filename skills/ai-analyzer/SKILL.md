...
### 2. 健康筛查与趋势提示
- **筛查工具**: 仅在名称、版本、适用人群和全部必需输入均已确认时使用
- **糖尿病筛查**: ADA Risk Test只生成筛查结果，不生成十年患病概率
- **心血管风险**: 只有适用的、当前且已验证的计算器才能生成百分比；否则仅整理已记录的风险因素
- **营养缺乏风险**: 基于RDA达成率和饮食模式分析
- **睡眠障碍风险**: 基于PSQI和睡眠模式分析

### 3. 个性化建议引擎
- **基础个性化**: 基于年龄、性别、BMI、活动水平等静态档案
- **建议分级**: Level 1（一般信息）、Level 2（供专业人员复核的问题）；不生成个体医疗建议
- **循证依据**: 基于医学指南和循证医学证据
- **可操作性**: 提供具体、可行的改进建议
...
先记录工具名称、版本、适用年龄/人群、排除条件和必需输入。缺少任何一项时，不计算个体概率。筛查分数必须标为筛查结果，不得改写为诊断或十年风险。对营养、睡眠和其他未验证项目，只报告观察到的数据与不确定性，并建议由合格专业人员解释。
...
- 不改变药物、检查、饮食或运动治疗方案；将个体决策交给合格专业人员

#### 步骤 8: 生成分析报告

**文本报告**: 包含总体评估、风险预测、关键趋势、相关性发现、个性化建议

**HTML报告**: 只有在项目中已有经过检查的报告生成器时才生成；否则输出结构化Markdown，不承诺不存在的脚本
...
### 筛查和风险工具
- 不把筛查评分转换为概率
- 只使用当前、具名、版本化且适用于该用户的验证计算器
- 在输出中记录输入、版本、适用性限制和计算日期；否则不生成分数

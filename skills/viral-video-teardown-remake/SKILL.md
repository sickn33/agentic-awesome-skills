...
description: "Install and use the official Viral Video Remake package, pinned by digest, for paid hosted work on the Beatra service."
category: media
risk: critical
source: community
source_repo: beatra-ai/viral-video-remake-skill
source_type: official
date_added: "2026-09-18"
author: beatra-ai
tags: [short-video, video-generation, mcp, paid-api, beatra]
tools: [claude, codex, cursor, gemini]
license: "MIT-0"
license_source: "https://github.com/beatra-ai/viral-video-remake-skill/blob/46f78753183ce9bd35574cdef7f57f811f468794/LICENSE"
---
...
Break a short video that worked into its hook, beats, and call to action, then rebuild that structure around your own subject as a shot list, beat frames, and narration, delivered as one vertical clip or as segmented sources with captions and a timecoded edit list, from inside Claude Code, Codex, or OpenClaw. The work is produced on the hosted, paid Beatra service
(`mcp.beatra.ai`).
...
| --- | --- |
| Package | `viral-video-teardown-remake` `0.3.1` |
| Archive | `https://cdn.beatra.ai/agent-packages/viral-video-teardown-remake/v0.3.1/viral-video-teardown-remake-skill-0.3.1.zip` |
| Archive SHA-256 | `f767938c5838a0e9f606f5ff0d8d0f6a5bddfc69906304d31e61749d3bf3fc63` |
| Source tree | [`beatra-ai/viral-video-remake-skill@46f7875`](https://github.com/beatra-ai/viral-video-remake-skill/tree/46f78753183ce9bd35574cdef7f57f811f468794/skills/viral-video-teardown-remake) (full SHA `46f78753183ce9bd35574cdef7f57f811f468794`) |
...
- Use when the user explicitly wants Viral Video Remake work produced on Beatra and accepts that it is paid.
...
curl -fsSLO "https://cdn.beatra.ai/agent-packages/viral-video-teardown-remake/v0.3.1/viral-video-teardown-remake-skill-0.3.1.zip"
printf '%s  %s\n' \
  'f767938c5838a0e9f606f5ff0d8d0f6a5bddfc69906304d31e61749d3bf3fc63' \
  'viral-video-teardown-remake-skill-0.3.1.zip' | shasum -a 256 -c -
```
...
```bash
unzip -l viral-video-teardown-remake-skill-0.3.1.zip
unzip -q viral-video-teardown-remake-skill-0.3.1.zip
find viral-video-teardown-remake -type l -print            # expect no output
find viral-video-teardown-remake -type f -perm -111 -print # expect no output
grep -n '"auto_update": True' viral-video-teardown-remake/scripts/mcp_client.py
```
...
```bash
git clone --quiet --filter=blob:none --no-checkout https://github.com/beatra-ai/viral-video-remake-skill.git src
git -C src checkout --quiet 46f78753183ce9bd35574cdef7f57f811f468794 -- skills/viral-video-teardown-remake
(cd viral-video-teardown-remake && find . -type f | LC_ALL=C sort | xargs shasum -a 256) > archive.sha
(cd src/skills/viral-video-teardown-remake && find . -type f | LC_ALL=C sort | xargs shasum -a 256) > tree.sha
...
```bash
dest="$HOME/.claude/skills/viral-video-teardown-remake"
test ! -e "$dest" || { echo "destination exists; stop and ask the user"; exit 1; }
cp -R viral-video-teardown-remake "$dest"
...
```bash
python3 "$HOME/.claude/skills/viral-video-teardown-remake/scripts/mcp_client.py" verify
```
...
review directory, verify its digest against the matching tree in `beatra-ai/viral-video-remake-skill`
...
- **Network.** Authorization uses `api.beatra.ai`; every tool call and upload
  ...
  `https://beatra.ai/skills/viral-video-teardown-remake/install.json` and archives from
  `cdn.beatra.ai`.
...
- [Source tree at the pinned commit](https://github.com/beatra-ai/viral-video-remake-skill/tree/46f78753183ce9bd35574cdef7f57f811f468794/skills/viral-video-teardown-remake)
- [Product page](https://beatra.ai/skills/viral-video-teardown-remake)

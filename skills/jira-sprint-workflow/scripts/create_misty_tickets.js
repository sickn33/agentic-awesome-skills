...
async function main() {
    ...
    const TEAM_ID = 'd83984d8-51c1-40f3-90b2-fa49e0a1c281';

    const filePath = path.resolve(__dirname, '../../../../Initiatives/1304_workflow_template/Workflow template - Misty_tasks.json');
    const content = fs.readFileSync(filePath, 'utf8');
    const parsedData = JSON.parse(content);
    let tasks = [];
    
    parsedData.tasks.forEach((t, i) => {
        let isDeleted = t.summary.startsWith('~~');
        ...
        tasks.push({
            id: i + 1,
            summary: t.summary.replace(/^~~|~~$/g, ''), // clean up summary
    ...
    for (let task of targetTasks) {
        ...
        try {
            ...
            if (e.statusCode === 400 && e.response) {
                ...
                console.error(`Failed to create task ${task.id}`, JSON.stringify(e));
            }
        }

...
description: Turn a theme, lyric, scene, or reference audio into reviewable songs, instrumentals, BGM, and soundtracks with a clear creative direction and focused revisions.
risk: critical
source: https://github.com/beatra-ai/beatra-skills/tree/v2026.08.31/skills/beatra-ai-music-creator
...
Official Beatra skill. Turn a theme, lyric, scene, or reference audio into reviewable songs, instrumentals, BGM, and soundtracks with a clear creative direction and focused revisions.

This entry is a catalog pointer. The executable package lives in
[`beatra-ai/beatra-skills`](https://github.com/beatra-ai/beatra-skills/tree/v2026.08.31/skills/beatra-ai-music-creator) at the immutable tag
...
- The user needs an original song, instrumental, background track, or soundtrack.
- A theme, lyric, scene description, or reference audio should drive the arrangement.
- An existing generated track needs a focused revision rather than a fresh start.
...
Expected digest for `beatra-ai-music-creator-skill-1.3.5.zip`:

```
d8407d683206a52835f970aca92cf91725698135376f213172c23340eb544cf5
```

**Step 2 — inspect before activating.**

```bash
find "$REVIEW/beatra-ai-music-creator" -type f | sort
find "$REVIEW/beatra-ai-music-creator" -type l          # expect no output: no symlinks
```
...
grep " skills/beatra-ai-music-creator/" SHA256SUMS.txt | shasum -a 256 -c -
```

## Update

Repeat steps 1 and 2 with the newer `VER`, verify the digest again, review the
diff against the installed copy, then replace `~/.claude/skills/beatra-ai-music-creator/`.
...
```bash
python3 ~/.claude/skills/beatra-ai-music-creator/scripts/uninstall.py   # revokes the stored token
rm -rf ~/.claude/skills/beatra-ai-music-creator
```
...
- Generates original audio. It does not clear, license, or reproduce existing commercial recordings.
- Generation is asynchronous and polled by task id.
- Reference audio is uploaded to Beatra for analysis.
...
- Archive SHA-256: `d8407d683206a52835f970aca92cf91725698135376f213172c23340eb544cf5`

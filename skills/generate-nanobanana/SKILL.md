...
Pick the model for the job and read its recipe file (`models/<model>.md`) before calling anything — recipes hold the exact request shape.

| Task | Model | Model ID | Ballpark cost |
| --- | --- | --- | --- |
| Image (draft) | Nano Banana 2 Lite | `gemini-3.1-flash-lite-image` | $0.03–0.05 |
| Image (standard) | Nano Banana 2 | `gemini-3.1-flash-image` | $0.07–0.15 |
| Image (quality, multi-image fusion) | Nano Banana Pro | `gemini-3-pro-image-preview` | $0.13–0.30 |
| Video | Gemini Omni Flash | `gemini-omni-flash-preview` | per-second, quoted before running |

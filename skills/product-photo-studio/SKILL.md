...
description: "Install and use the official AI Product Photography package, pinned by digest, for paid hosted work on the Beatra service."
category: media
risk: critical
source: community
source_repo: beatra-ai/ai-product-photography-skill
source_type: official
date_added: "2026-09-18"
author: beatra-ai
tags: [product-photography, ecommerce, text-to-image, mcp, paid-api, beatra]
tools: [claude, codex, cursor, gemini]
license: "MIT-0"
license_source: "https://github.com/beatra-ai/ai-product-photography-skill/blob/1490364794606dfbedd629cb3ab886271b5829e4/LICENSE"
---
...
Turn a real product photo into studio-quality ecommerce images and lifestyle scenes, from inside Claude Code, Codex, or OpenClaw. The work is produced on the hosted, paid Beatra service
(`mcp.beatra.ai`).
...
| --- | --- |
| Package | `product-photo-studio` `0.2.0` |
| Archive | `https://cdn.beatra.ai/agent-packages/product-photo-studio/v0.2.0/product-photo-studio-skill-0.2.0.zip` |
| Archive SHA-256 | `460a594154a04a7da9e06ab5d476d3e67d33ba9f383cdb8a5a86860fd0c7925e` |
| Source tree | [`beatra-ai/ai-product-photography-skill@1490364`](https://github.com/beatra-ai/ai-product-photography-skill/tree/1490364794606dfbedd629cb3ab886271b5829e4/skills/product-photo-studio) (full SHA `1490364794606dfbedd629cb3ab886271b5829e4`) |
...
- Use when the user explicitly wants AI Product Photography work produced on Beatra and accepts that it is paid.
...
curl -fsSLO "https://cdn.beatra.ai/agent-packages/product-photo-studio/v0.2.0/product-photo-studio-skill-0.2.0.zip"
printf '%s  %s\n' \
  '460a594154a04a7da9e06ab5d476d3e67d33ba9f383cdb8a5a86860fd0c7925e' \
  'product-photo-studio-skill-0.2.0.zip' | shasum -a 256 -c -
```
...
```bash
unzip -l product-photo-studio-skill-0.2.0.zip
unzip -q product-photo-studio-skill-0.2.0.zip
find product-photo-studio -type l -print            # expect no output
find product-photo-studio -type f -perm -111 -print # expect no output
grep -n '"auto_update": True' product-photo-studio/scripts/mcp_client.py
```
...
```bash
git clone --quiet --filter=blob:none --no-checkout https://github.com/beatra-ai/ai-product-photography-skill.git src
git -C src checkout --quiet 1490364794606dfbedd629cb3ab886271b5829e4 -- skills/product-photo-studio
(cd product-photo-studio && find . -type f | LC_ALL=C sort | xargs shasum -a 256) > archive.sha
(cd src/skills/product-photo-studio && find . -type f | LC_ALL=C sort | xargs shasum -a 256) > tree.sha
...
```bash
dest="$HOME/.claude/skills/product-photo-studio"
test ! -e "$dest" || { echo "destination exists; stop and ask the user"; exit 1; }
cp -R product-photo-studio "$dest"
...
```bash
python3 "$HOME/.claude/skills/product-photo-studio/scripts/mcp_client.py" verify
```
...
review directory, verify its digest against the matching tree in `beatra-ai/ai-product-photography-skill`
...
- **Network.** Authorization uses `api.beatra.ai`; every tool call and upload
  ...
  `https://beatra.ai/skills/product-photo-studio/install.json` and archives from
  `cdn.beatra.ai`.
...
- [Source tree at the pinned commit](https://github.com/beatra-ai/ai-product-photography-skill/tree/1490364794606dfbedd629cb3ab886271b5829e4/skills/product-photo-studio)
- [Product page](https://beatra.ai/skills/product-photo-studio)

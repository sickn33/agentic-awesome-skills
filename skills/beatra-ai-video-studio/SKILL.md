...
description: Generate, animate, edit, and extend short AI videos for ads, product stories, social posts, b-roll, transitions, and cinematic concepts.
risk: critical
source: https://github.com/beatra-ai/beatra-skills/tree/v2026.08.31/skills/beatra-ai-video-studio
...
Official Beatra skill. Generate, animate, edit, and extend short AI videos for ads, product stories, social posts, b-roll, transitions, and cinematic concepts.

This entry is a catalog pointer. The executable package lives in
[`beatra-ai/beatra-skills`](https://github.com/beatra-ai/beatra-skills/tree/v2026.08.31/skills/beatra-ai-video-studio) at the immutable tag
...
- The user needs a short AI video for an ad, product story, social post, or b-roll.
- A still image should be animated, or an existing clip extended or restyled.
- The deliverable is video and should not open the image or music workflows.
...
Expected digest for `beatra-ai-video-studio-skill-1.2.4.zip`:

```
19d4c705e40fab6665f4de107314e6bfdd4bcac894ef3ea4e5394598b68e6593
```

**Step 2 — inspect before activating.**

```bash
find "$REVIEW/beatra-ai-video-studio" -type f | sort
find "$REVIEW/beatra-ai-video-studio" -type l          # expect no output: no symlinks
```
...
grep " skills/beatra-ai-video-studio/" SHA256SUMS.txt | shasum -a 256 -c -
```

## Update

Repeat steps 1 and 2 with the newer `VER`, verify the digest again, review the
diff against the installed copy, then replace `~/.claude/skills/beatra-ai-video-studio/`.
...
```bash
python3 ~/.claude/skills/beatra-ai-video-studio/scripts/uninstall.py   # revokes the stored token
rm -rf ~/.claude/skills/beatra-ai-video-studio
```
...
- Short-form output. Not a timeline editor and not a substitute for an NLE.
- Generation is asynchronous; a clip can take minutes and is polled by task id.
...
- Archive SHA-256: `19d4c705e40fab6665f4de107314e6bfdd4bcac894ef3ea4e5394598b68e6593`

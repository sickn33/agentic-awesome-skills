...
description: "Install and use the official AI Music Generator package, pinned by digest, for paid hosted work on the Beatra service."
category: media
risk: critical
source: community
source_repo: beatra-ai/ai-music-generator-skill
source_type: official
date_added: "2026-09-18"
author: beatra-ai
tags: [music-generation, text-to-music, mcp, paid-api, beatra]
tools: [claude, codex, cursor, gemini]
license: "MIT-0"
license_source: "https://github.com/beatra-ai/ai-music-generator-skill/blob/fee8fbf3dcd8a723bee1e372915c6f78593f9b35/LICENSE"
---
...
Turn a theme, lyrics, or reference audio into songs, instrumentals, and soundtracks, from inside Claude Code, Codex, or OpenClaw. The work is produced on the hosted, paid Beatra service
(`mcp.beatra.ai`).
...
| --- | --- |
| Package | `music-generation-studio` `0.1.8` |
| Archive | `https://cdn.beatra.ai/agent-packages/music-generation-studio/v0.1.8/music-generation-studio-skill-0.1.8.zip` |
| Archive SHA-256 | `8cd20e3c8c021828129c257343976e38145f838c1c65c7ba1b5e55076c4180c5` |
| Source tree | [`beatra-ai/ai-music-generator-skill@fee8fbf`](https://github.com/beatra-ai/ai-music-generator-skill/tree/fee8fbf3dcd8a723bee1e372915c6f78593f9b35/skills/music-generation-studio) (full SHA `fee8fbf3dcd8a723bee1e372915c6f78593f9b35`) |
...
- Use when the user explicitly wants AI Music Generator audio produced on Beatra and accepts that it is paid.
- Use when they have agreed to send lyrics, themes, or reference audio to a third party.
- Do not use for local DAW work or when no paid render was approved.
...
curl -fsSLO "https://cdn.beatra.ai/agent-packages/music-generation-studio/v0.1.8/music-generation-studio-skill-0.1.8.zip"
printf '%s  %s\n' \
  '8cd20e3c8c021828129c257343976e38145f838c1c65c7ba1b5e55076c4180c5' \
  'music-generation-studio-skill-0.1.8.zip' | shasum -a 256 -c -
```
...
```bash
unzip -l music-generation-studio-skill-0.1.8.zip
unzip -q music-generation-studio-skill-0.1.8.zip
find music-generation-studio -type l -print            # expect no output
find music-generation-studio -type f -perm -111 -print # expect no output
grep -n '"auto_update": True' music-generation-studio/scripts/mcp_client.py
```
...
```bash
git clone --quiet --filter=blob:none --no-checkout https://github.com/beatra-ai/ai-music-generator-skill.git src
git -C src checkout --quiet fee8fbf3dcd8a723bee1e372915c6f78593f9b35 -- skills/music-generation-studio
(cd music-generation-studio && find . -type f | LC_ALL=C sort | xargs shasum -a 256) > archive.sha
(cd src/skills/music-generation-studio && find . -type f | LC_ALL=C sort | xargs shasum -a 256) > tree.sha
...
```bash
dest="$HOME/.claude/skills/music-generation-studio"
test ! -e "$dest" || { echo "destination exists; stop and ask the user"; exit 1; }
cp -R music-generation-studio "$dest"
...
```bash
python3 "$HOME/.claude/skills/music-generation-studio/scripts/mcp_client.py" verify
```
...
review directory, verify its digest against the matching tree in `beatra-ai/ai-music-generator-skill`
...
- **Network.** Authorization uses `api.beatra.ai`; every tool call and upload
  ...
  `https://beatra.ai/skills/music-generation-studio/install.json` and archives from
  `cdn.beatra.ai`.
...
- [Source tree at the pinned commit](https://github.com/beatra-ai/ai-music-generator-skill/tree/fee8fbf3dcd8a723bee1e372915c6f78593f9b35/skills/music-generation-studio)
- [Product page](https://beatra.ai/skills/music-generation-studio)

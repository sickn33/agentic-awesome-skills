...
7. Wait for publishing workflows, then bind every proof to the exact released commit: verify the tag/ref, GitHub Release, npm version and intended dist-tag, required CI, CodeQL, and the release-tag Pages build from the exact immutable `vX.Y.Z` tag. Never manually dispatch the release-tag build from `main` or another branch. Protected pushes to `main` use the separate automatic Pages path, which checks current-main identity before build and again before deploy; `[skip pages]` canonical-sync commits remain excluded. Verify live `llms.txt`, `skills.json`, catalog and plugin routes, and the legacy redirect bridge; do not accept a successful run for a different SHA.

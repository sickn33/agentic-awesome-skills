...
- **治疗时间线对照**：描述治疗开始前后的记录变化，不推断治疗导致改善
...
### 4. 描述性洞察
- **趋势摘要**：基于已记录数据描述变化
- **不确定性**：标注数据不足、混杂因素和测量变化
- **升级提示**：对急性症状或异常结果建议及时由合格专业人员评估，不进行预测
...
// 参考范围标记（必须使用报告随附范围，不能诊断）
function checkThresholds(value, thresholds) {
  // 标记需要专业解释的记录，并保留单位、来源和测量日期
}
```
...
```javascript
// 只报告趋势、时间关系、数据质量和不确定性。
// 不判断药物有效性，不改变治疗，不预测疾病。
```
...
**HTML文件特点**：
- ⚠️ 使用CDN时需要网络并会向第三方暴露请求元数据；离线或敏感场景应使用经审核的本地依赖
- ✅ 交互式图表（缩放、导出、图例切换）
- ✅ 响应式设计（移动端适配）
- ✅ 可打印（打印优化样式）
- ✅ 可分享（发送给医生）
...
数据结构必须从实际输入文件和schema确认；本技能不承诺额外的数据源文档。
...
使用的算法、参数、版本和限制必须记录在报告中；本技能不承诺额外的算法文档。
...
- ✅ 默认不上传健康数据
- ⚠️ CDN或其他网络依赖必须经授权，并披露目的地和传输的元数据
- ✅ 分析结果仅保存在本地
- ✅ 只有完全本地打包且验证无网络请求的HTML才能称为独立运行
...
### 示例 4：药物时间线
**用户**："开始服药后记录有什么变化？"
**输出**：描述药物开始日期前后的读数和缺失数据，明确时间关联不证明疗效，并建议由处方医生解释

#!/usr/bin/env python3
"""Fail-closed compatibility stub for unimplemented architecture analysis."""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Compatibility stub; automated architecture analysis is not implemented."
    )
    parser.add_argument("target", type=workspace_path, help="existing path under the current workspace")
    args = parser.parse_args()
    print(
        f"unsupported: no analysis or fixes were performed for {args.target}; follow the SKILL.md review workflow",
        file=sys.stderr,
    )
    return 2

...
- 评估食物和饮水安全
- 确认环境风险(高温、高原等)
- 仅在访问并标注时间戳的官方实时来源后报告当前疫情；否则明确数据可能过期
- 提供WHO/CDC参考链接
...
**时间规划原则**:
- 尽早预约旅行医学专业人员；具体疫苗、剂次和时间取决于目的地、行程、既往免疫和个人禁忌
- 不从静态模板判定必需、推荐或禁忌，也不自行安排接种
...
- 多西环素 vs 抗酸药、钙铁补充剂
- 甲氟喹 vs 某些心脏病药物
- 利福平/利福布汀等特定诱导剂可能影响激素避孕；不要把大多数广谱抗生素笼统标为降低疗效，具体相互作用交由药师或医生核对
...
**数据更新**: 静态数据只能说明历史或一般风险，不能支持“当前疫情”或现行入境要求。每次相关输出必须查询目的地官方信息、WHO/CDC或当地卫生部门并记录URL和访问时间；无法联网时停止该部分。
...
### 数据保护
- 默认不收集或存储护照号码；如业务确需存储，必须先定义授权、最小化、加密算法、密钥管理、访问控制、保留期和删除流程
- 二维码不包含完整敏感信息
- 支持数据导出和删除

#!/usr/bin/env python3
"""Fail-closed compatibility stub for an unimplemented diagram generator."""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Compatibility stub; architecture diagram generation is not implemented."
    )
    parser.add_argument("target", type=workspace_path, help="existing path under the current workspace")
    args = parser.parse_args()
    print(
        f"unsupported: no diagram was generated for {args.target}; follow the SKILL.md review workflow",
        file=sys.stderr,
    )
    return 2

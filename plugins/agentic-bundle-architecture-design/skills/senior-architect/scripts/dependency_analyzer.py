#!/usr/bin/env python3
"""Fail-closed compatibility stub for unimplemented dependency analysis."""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Compatibility stub; automated dependency analysis is not implemented."
    )
    parser.add_argument("target", type=workspace_path, help="existing path under the current workspace")
    args = parser.parse_args()
    print(
        f"unsupported: no dependency findings were produced for {args.target}; use project-specific read-only tooling",
        file=sys.stderr,
    )
    return 2

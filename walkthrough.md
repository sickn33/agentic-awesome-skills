...
# Contribution Walkthrough - 2026-09-01 Evidence-First AGENTS.md Guidance

- Reworked `skills/agents-md/SKILL.md` around a bounded, repository-evidence-first create/revise/audit workflow.
- Removed universal requirements for symlinks, commit attribution, section names, and line limits so the skill preserves maintainer intent and tool-specific differences.
- Added scoped monorepo guidance, focused validation, examples, explicit safety boundaries, failure modes, and limitations.
- Preserved the existing community provenance and critical risk classification, and added the original import date plus a development category.
- Added an optional, privacy-gated independent checker while keeping repository evidence and local validation authoritative.
